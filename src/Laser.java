...
public class Laser implements Drawable {
    ...
    public void draw(Graphics2D g) {
        ...
        if (!isFiring) {
            ...
            lineColor = new Color(255, green, 0);
        } else {
            lineColor = Color.RED;
        }
        ...
        for (int i = 0; i < segments.size(); i++) {
            ...
            double segRatio = (double)i / segments.size();

            if (isFiring && elapsed > 1000 && segRatio <= tracerProgress) {
    ...
    private boolean lineIntersectsRect(double x1, double y1,
                                        double x2, double y2, Rectangle r) {
        double dist = Math.hypot(x2 - x1, y2 - y1);
        for (int i = 0; i <= dist; i += 5) {
            double ratio = i / dist;
            if (r.contains(x1 + (x2 - x1) * ratio,
                           y1 + (y2 - y1) * ratio)) return true;
        }
        return false;
    }
}